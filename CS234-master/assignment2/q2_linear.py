...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, shape = (config.batch_size, img_height, img_width, nchannels*config.state_history))
        self.a = tf.placeholder(tf.int32, shape = config.batch_size)
        self.r = tf.placeholder(tf.float32, shape = config.batch_size)
        self.sp = tf.placeholder(tf.uint8, shape = (config.batch_size, img_height, img_width, nchannels*config.state_history))
